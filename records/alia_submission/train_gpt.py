"""
train_gpt_submit.py — Submission: PR #315 base + Reptile meta-TTT
Architecture: 11L, Partial RoPE, LN Scale, Late QAT, XSA4, EMA, int6+zstd
Novel addition: Reptile meta-learning (last 20% of training) for test-time adaptation
"""
...
class Hyperparameters:
    ...
    # Reptile meta-learning config
    reptile_enabled = bool(int(os.environ.get("REPTILE_ENABLED", "1")))
    reptile_time_frac = float(os.environ.get("REPTILE_TIME_FRAC", 0.2))
    reptile_inner_steps = int(os.environ.get("REPTILE_INNER_STEPS", 3))
    reptile_inner_lr = float(os.environ.get("REPTILE_INNER_LR", 0.1))
    reptile_outer_lr = float(os.environ.get("REPTILE_OUTER_LR", 0.01))
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        y = F.scaled_dot_product_attention(q, k, v, is_causal=True) #y = flash_attn_3_func(q, k, v, causal=True)
        if self.use_xsa:
...
def _get_reptile_param_names(raw_model: GPT) -> list[str]:
    """Identify Reptile params: MLP layers of last 1/4 blocks."""
    num_blocks = len(raw_model.blocks)
    suffix_start = num_blocks - num_blocks // 4
    return [name for name, _ in raw_model.named_parameters()
...
def main() -> None:
    ...
    log0(f"xsa_last_n:{args.xsa_last_n} rope_dims:{args.rope_dims} ln_scale:{args.ln_scale} late_qat:{args.late_qat} ema_enabled:{args.ema_enabled}")
    log0(f"reptile:enabled={args.reptile_enabled} time_frac={args.reptile_time_frac} inner_steps={args.reptile_inner_steps}")
    ...
    phase1_wallclock_seconds = args.max_wallclock_seconds * (1.0 - args.reptile_time_frac) if args.reptile_enabled else args.max_wallclock_seconds
    max_wallclock_ms = 1000.0 * phase1_wallclock_seconds if phase1_wallclock_seconds > 0 else None
    ...
    log0(f"phase1:starting normal training (budget={phase1_wallclock_seconds:.1f}s)")
    ...
    if args.reptile_enabled:
        reptile_budget_s = args.max_wallclock_seconds * args.reptile_time_frac
        log0(f"phase2:starting Reptile meta-learning (budget={reptile_budget_s:.1f}s)")
        reptile_start = time.perf_counter()

        raw_model = base_model.module if hasattr(base_model, 'module') else base_model
        reptile_param_names = _get_reptile_param_names(raw_model)
        reptile_params = {name: p for name, p in raw_model.named_parameters() if name in reptile_param_names}
        log0(f"phase2:reptile params: {len(reptile_param_names)} tensors")

        base_state = {name: p.data.clone() for name, p in reptile_params.items()}

        reptile_step = 0
        train_data = TokenStream(args.train_files)

        while (time.perf_counter() - reptile_start) < reptile_budget_s:
            saved = {name: p.data.clone() for name, p in reptile_params.items()}

            raw_model.train()
            for inner_step in range(args.reptile_inner_steps):
                chunk = train_data.take(args.train_seq_len + 1)
                local = chunk.to(dtype=torch.int64)
                x = local[:-1].unsqueeze(0).to(device)
                y = local[1:].unsqueeze(0).to(device)
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    loss = raw_model(x, y)
                loss.backward()
                with torch.no_grad():
                    for name, param in reptile_params.items():
        ...
        elapsed_reptile = time.perf_counter() - reptile_start
        log0(f"phase2:completed {reptile_step} Reptile meta-steps in {elapsed_reptile:.1f}s")
